...
server -host <hostname> -srvcert <serverCertFile> -cacert <caCertFile> -srvkey <serverPrivateKeyFile> [-port <port> -certopt <certopt> -help]
...
Options:
  ...
  -certopt    Optional, specifies the option for authenticating a client via certificate:
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		resp := fmt.Sprintf("Ciao, %s bello", body)
		w.Write([]byte(resp))
		log.Printf("Server: Sent response %s", resp)
	})
